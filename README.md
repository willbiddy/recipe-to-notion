...
Each recipe costs about **2.5 cents** in Claude API usage, so you can save roughly **40 recipes** for **$1**. The default model is Sonnet, but you can change it by setting the `CLAUDE_MODEL` environment variable to `"haiku"`, `"sonnet"`, or `"opus"`.

When you save multiple recipes within 5 minutes, subsequent recipes cost about 40% less because Claude caches the system prompt.
