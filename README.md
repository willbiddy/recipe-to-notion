...
Each recipe costs roughly **2-3 cents** in Claude API usage (roughly 4,300-5,300 input tokens and 400-1,000 output tokens per recipe). The default model is Sonnet 4.6, but you can change it by setting the `CLAUDE_MODEL` environment variable to `"haiku"`, `"sonnet"`, or `"opus"`.

**Prompt caching:** When you save multiple recipes within 5 minutes, subsequent recipes cost about 40% less because Claude caches the system prompt.
